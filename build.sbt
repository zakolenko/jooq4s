...
lazy val unidocSettings = Seq(
  ...
  scalacOptions in (ScalaUnidoc, unidoc) ++=
    Opts.doc.title(s"jooq4s"),
  scalacOptions in (ScalaUnidoc, unidoc) ++=
    Opts.doc.sourceUrl(s"https://github.com/zakolenko/jooq4s/tree/v${version.value}€{FILE_PATH}.scala"),
  scalacOptions in (ScalaUnidoc, unidoc) ++=
...
lazy val sharedSettings = Seq(
  gitHubRepositoryID := "zakolenko/jooq4s",
  ...
  homepage := Some(url(s"https://zakolenko.github.io/jooq4s/")),
...
lazy val jooq4s = project.in(file("."))
  .enablePlugins(ScalaUnidocPlugin)
  .aggregate(core, hikari)
  .configure(profile)
...
lazy val site = project.in(file("site"))
  ...
  .settings(sharedSettings)
  .settings(doNotPublishArtifact)
  .dependsOn(core, hikari)
  .settings{
    import microsites._
    Seq(
      micrositeName := "jooq4s",
      micrositeDescription := "Scala wrapper for jOOQ",
      micrositeAuthor := "Roman Zakolenko",
      micrositeGithubOwner := "zakolenko",
      micrositeGithubRepo := "jooq4s",
      micrositeUrl := "https://zakolenko.github.io",
      micrositeBaseUrl := "/jooq4s/".replaceAll("[/]+$", ""),
      micrositeDocumentationUrl := s"https://zakolenko.github.io/jooq4s/api/",
      micrositeGitterChannelUrl := "zakolenko/jooq4s",
      ...
      addMappingsToSiteDir(mappings in (ScalaUnidoc, packageDoc) in jooq4s, docsMappingsAPIDir),
